 		...
 		$.ajax({
			...
			success:function(data){
				if(data!="404"){
					...
					showMessage("成功获取QQ昵称！",2500);
				}else{
					showMessage("好像获取失败了，是不是没有开启https啊?",2500);
				}
...
$(document).ready(function(){
	...
	var play=function(){
		...
		$("#play-music i.iconfont").eq(1).show();
		$("#play-music i.iconfont").eq(0).hide();
		$("#song_img img").addClass("rog");
		music.play();
		showMessage("开始播放...",2500);
	};
	var pause=function(){
		$("#song_play").removeClass("is_playing");
		$("#play-music i.iconfont").eq(0).show();
		$("#play-music i.iconfont").eq(1).hide();
		$("#song_img img").removeClass("rog");
		music.pause();
		showMessage("暂停播放...",2500);
	}
	if(playstyle==0){
...
$("audio#music").on("ended",function(){
	...
	if(wife_var.open=="yes"&&!is_mobile()){
    ...
    $("#recover-position").click(function(){
       	//$("#wife").remove();
        //Init_wife();
        $("#wife").removeAttr("style").css({right:0,bottom:0});
    ...
    $("#play-music").click(function(){
    	...
    	if(_this.hasClass('is_playing')){
    		$(this).find("i.iconfont").eq(1).hide();
    		$(this).find("i.iconfont").eq(0).show();
			pause();
		}
		else{
			_this.addClass("is_playing");
			$(this).find("i.iconfont").eq(0).hide();
    		$(this).find("i.iconfont").eq(1).show();
			play();
		}
    });
