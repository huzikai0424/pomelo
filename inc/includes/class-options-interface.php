...
class Options_Framework_Interface {
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case 'textarea_music':
			$url=THEME_URL."playlist.json";
			$val = file_get_contents($url);
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" style="overflow: hidden;" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" rows="8"' . $placeholder . 'readonly="readonly"'.'>' . esc_textarea( $val ) . '</textarea>';
			break;
